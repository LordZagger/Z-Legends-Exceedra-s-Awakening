...
# Background Images
background_title = pygame.image.load("house.png")  # Title screen background
...
battle_background = pygame.image.load("hill.png")  # Battle scene background
battle_background = pygame.transform.scale(battle_background, (SCREEN_WIDTH, SCREEN_HEIGHT))

# Character Images
main_character = pygame.image.load("exceedra1.png")  # Main character sprite
main_character = pygame.transform.scale(main_character, (150, 150))

enemy = pygame.image.load("akobos.png")  # Enemy sprite
...
def make_button(text,font,text_size,text_color,x,y,button_SCREEN_WIDTH,button_height,button_color,highlight_color,action=None,can_cancel_move=False,events=None):
    ...
    while True:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if can_cancel_move == True and Character.battleOn:
                    ...
                    if action != None and Move.Move_isUnlocked:
                        if x <= mouse[0] <= x+button_SCREEN_WIDTH and y <= mouse[1] <= y+button_height and (event.button == 1 or event.button == 2):
        ...
        if x <= mouse[0] <= x+button_SCREEN_WIDTH and y <= mouse[1] <= y+button_height:
            #switch button color to highlight_color
            pygame.draw.rect(screen,highlight_color,(x,y,button_SCREEN_WIDTH,button_height))
        
        else:
            #draw button at position x,y and with dimensions button_SCREEN_WIDTH and button_height
            pygame.draw.rect(screen,button_color,(x,y,button_SCREEN_WIDTH,button_height))
            
        make_text(text,font,text_size,text_color,x+button_SCREEN_WIDTH/2,y+button_height/2)
...
def Battle(character1,character2,background,music,next_scene):
    ...
    while True:
        ...
        character1.BattlePosition(100,SCREEN_HEIGHT-300)
        character2.BattlePosition(SCREEN_WIDTH-250,SCREEN_HEIGHT-300)
        ...
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_1:
                    ...
                    Guard(character1,character2)
                elif event.key == pygame.K_3:
                    Recover(character1,character2)
...
def battle_screen():
    ...
    # Draw action buttons
    button_SCREEN_WIDTH, button_SCREEN_HEIGHT = 150, 50
    button_spacing = 20
    button_y = SCREEN_HEIGHT - 100

    attack_rect = pygame.Rect(50, button_y, button_SCREEN_WIDTH, button_SCREEN_HEIGHT)
    guard_rect = pygame.Rect(50 + button_SCREEN_WIDTH + button_spacing, button_y, button_SCREEN_WIDTH, button_SCREEN_HEIGHT)
    recovery_rect = pygame.Rect(50 + 2 * (button_SCREEN_WIDTH + button_spacing), button_y, button_SCREEN_WIDTH, button_SCREEN_HEIGHT)
            ...
            if not battle_mode:
                if event.key == pygame.K_RETURN:  # On Enter key
                    if scene == "start_menu":
                        ...
                        Battle(ExceedraMain,Hydranoid,school,BattleTheme1,Episode1_2_2)
                elif event.key == pygame.K_BACKSPACE:  # On Backspace key
                    if dialogue_index > 0:
                        dialogue_index -= 1  # Go to previous line
                    else:
